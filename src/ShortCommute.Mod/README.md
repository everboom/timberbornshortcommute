...
| file | role |
|---|---|
| `ShortCommuteConfigurator.cs` | `[Context("Game")]` Bindito wiring: binds `CommuteOptimizer` and registers it as a `TemplateModule` decorator on every `DistrictCenter`, plus a `CommuteCost` decorator on every `Worker`. No Harmony. |
